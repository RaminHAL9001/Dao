...
data PPrintItem
  ...
  | PPrintIndent  PPrintItem
  | PPrintString  { pPrintString :: UStr } -- ^ a single string of output
  | PPrintBlock   { pPrintItems :: [PPrintItem] } -- ^ print each item on it's own line
...
initPPrintState = PPrintState{ pPrintBlock = [] }

instance Monoid PPrintState where
  mempty      = initPPrintState
  mappend a b = PPrintState{ pPrintBlock = mappend (pPrintBlock a) (pPrintBlock b) }
...
pPush item = modify (\p -> p{ pPrintBlock = pPrintBlock p ++ [item] })
...
pEval fn = pPrintBlock (execState fn initPPrintState)
...
linesFromPPrintState maxWidth ps = end (execState (mapM_ prin (pPrintBlock ps)) mempty) where
  prin :: PPrintItem -> State Printer ()
  prin p = case p of
    PNewLine        -> newline
    PPrintString p  -> string (ulength p) (uchars p)
    PPrintIndent p  -> setIndent (+1) >> prin p >> setIndent (\x -> x-1)
    PPrintBlock  px -> do
    ...
    PPrintList    sep     px -> forM_ px $ \p ->
      prin p >> string (ulength sep) (uchars sep) >> softNewline
