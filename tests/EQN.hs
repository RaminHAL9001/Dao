...
data EQN = OR | AND | DEL | TERM ISet (Iv.Set I) | INV [EQN]
...
evalEqn :: [EQN] -> (ISet, Iv.Set I)
evalEqn eqn = case scan eqn of
  [INV  q  ] -> let (w, s) = evalEqn q in (complement w, Iv.invert s)
  [TERM w s] -> (w, s)
  ...
  where
    ...
    term bitF setF a b = Just (TERM (bitF ab bb) (setF as bs)) where
      (ab, as) = evalEqn [a]
      (bb, bs) = evalEqn [b]
    and a AND b = term (*) Iv.intersect a b
    and _ _   _ = Nothing
    del a DEL b = term (-) Iv.delete a b
    del _ _   _ = Nothing
    or  a OR  b = term (+) Iv.union a b
    or  _ _   _ = Nothing
...
results :: (ISet, Iv.Set I) -> (ISet, ISet)
...
showResults (eqnNum, eqn, i, e) =
  show eqnNum++": "++showEqn eqn++"\n\tbits "++show i++" doesnt match Iv.fromList "++show e
