...
data EQN = OR | AND | DEL | TERM ISet (EnumSet I) | INV [EQN]
...
evalEqn :: [EQN] -> (ISet, EnumSet I)
evalEqn eqn = case scan eqn of
  [INV  q  ] -> let (w, s) = evalEqn q in (complement w, setInvert s)
  [TERM w s] -> (w, s)
  ...
  where
    ...
    term bitF setF a b = Just (TERM (bitF ab bb) (setF as bs)) where
      (ab, as) = evalEqn [a]
      (bb, bs) = evalEqn [b]
    and a AND b = term (*) setIntersect a b
    and _ _   _ = Nothing
    del a DEL b = term (-) setDelete a b
    del _ _   _ = Nothing
    or  a OR  b = term (+) setUnion a b
    or  _ _   _ = Nothing
...
results :: (ISet, EnumSet I) -> (ISet, ISet)
...
showResults (eqnNum, eqn, i, e) =
  show eqnNum++": "++showEqn eqn++"\n\tbits "++show i++" doesnt match enumSet "++show e
