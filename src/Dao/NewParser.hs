...
--    infinitely and hang.
-- 
-- The line positions of tokens is computed by the 'lexicalAnalysis' phase, which assumes that
-- *every token* generated by a 'GenTokenizer' will return a token containing exactly the characters
-- lexed-off of the input string, nothing more and nothing less. It is therefore important that
-- space characters also be treated as tokens, and that no characters are thrown-away during lexing.
-- Lexers can return more than one token, so if necessary, return an 'Ignored' token along with a
-- token containing the important characters, then drop all 'Ignored' tokens during
-- 'syntacticAnalysis'.
...
lexInlineCComment :: Tokenizer
lexInlineCComment = lexString "/*" >> lexUntilTermStr "" "*/" >> makeToken ComInln

lexEndlineCComment :: Tokenizer
lexEndlineCComment = lexString "//" >> lexUntil (=='\n') >> makeToken ComEndl

lexInlineHaskellComment :: Tokenizer
lexInlineHaskellComment = lexString "{-" >> lexUntilTermStr "" "-}" >> makeToken ComInln
...
lexStringLiteral = lexChar '"' >> lexUntilTermChar '\\' '"' >> makeToken StrLit
...
lexNumber = do
  getDot <- msum $
    [ lexChar '0' >> lexCharP (charSet "xXbB") >> lexWhile isHexDigit >> return True
    , lexChar '.' >> lexWhile isDigit >> return False
    , lexWhile isDigit >> return True
    ]
  lexOptional ((if getDot then lexChar '.' else return ()) >> lexWhile isHexDigit)
  lexOptional (lexCharP (charSet "Ee") >> lexOptional (lexCharP (charSet "-+")) >> lexWhile isDigit)
  makeToken Number
...
lexHaskellLabel = loop 0 where
  ...
  final    = mplus (lexCharP isAlpha_ >> lexOptional (lexWhile isAlphaNum_) >> done) oper
  oper     = lexChar '(' >> lexWhile (\c -> c/=')' && isPunctuation c) >> lexChar ')' >> done
...
-- by this function.
tokenizeCLike :: String -> [Tokenizer]
tokenizeCLike operators =
  [ lexStringLiteral
  , lexCharLiteral
  , lexInlineCComment
  , lexEndlineCComment
