...
bindRandChoice (RandChoice arr) f = RandChoice $ fmap (fmap (\o -> o >>= runRandChoice . f)) arr
...
class HasRandGen o where
  randO :: RandO o
  randO = runRandChoice randChoice
  randChoice :: RandChoice o
  randChoice = randChoiceList [randO]

runRandChoice :: RandChoice o -> RandO o
runRandChoice (RandChoice{ getChoiceArray=arr }) = case arr of
  Nothing  -> fail "null RandChoice"
  Just arr -> let (lo, hi) = bounds arr in join $ ((arr!) . (lo+)) <$> nextInt (hi-lo)

-- | Used to instnatiate a default 'randO' function from a hand-written 'randChoice' instance. This
-- instance will define 'randO' as such:
-- > 'countNode' $ 'runRandChoice' 'randChoice'
countRunRandChoice :: HasRandGen o => RandO o
countRunRandChoice = countNode $ runRandChoice randChoice

-- | Used to instnatiate a default 'randO' function from a hand-written 'randChoice' instance. This
-- instance will define 'randO' as such:
-- > 'recurse' defaultValue $ 'runRandChoice' 'randChoice'
-- where @defaultValue@ is the first parameter to this function
recurseRunRandChoice :: HasRandGen o => o -> RandO o
recurseRunRandChoice nullval = recurse nullval $ runRandChoice randChoice
